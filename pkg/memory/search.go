...
func GetWritableAddrRanges(pid int) ([][2]int, error) {
	addrRanges := [][2]int{}
	var bufferStr, _ = EnumRegion(pid)
	scanner := bufio.NewScanner(strings.NewReader(bufferStr))
	for scanner.Scan() {
		line := scanner.Text()
...
func FindDataInAddrRanges(pid int, targetBytes []byte, addrRanges [][2]int) ([]int, error) {
	...
	for _, s := range addrRanges {
		...
		for i := 0; i < (memSize/splitSize)+1; i++ {
			...
			ReadMemory(pid, b, splittedBeginAddr, splittedEndAddr)
